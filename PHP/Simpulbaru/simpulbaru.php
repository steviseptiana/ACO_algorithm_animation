<?php
	...
	class simpulbaru
	{
		...
		{
			$hasil=array();
			$query=mysqli_query($this->koneksi,"SELECT id, simpul, (3959 * acos (cos ( radians('$lat') )* cos( radians( lat ) )* cos( radians( lng ) - radians('$lng') )+ sin ( radians('$lat') ) * sin( radians( lat ) ))) AS distance FROM koordinatawalsementara HAVING distance < 6000000000000 ORDER BY distance LIMIT 0 , 5");
		...
		function savedb($array)
		{
			...
			if (mysqli_num_rows($query1)==1)
			{
				...
				$querydelete1=mysqli_query($this->koneksi,"DELETE from graphsementara  WHERE simpul_awal=".$array['simpul1']." AND simpul_tujuan=".$array['simpul2']);
			}
			if (mysqli_num_rows($query2)==1)
			{
				...
				$querydelete1=mysqli_query($this->koneksi,"DELETE from graphsementara  WHERE simpul_awal=".$array['simpul2']." AND simpul_tujuan=".$array['simpul1']);
			}

			//$query3=mysqli_query($this->koneksi,"SELECT * from graph WHERE simpul_awal=".$array['koordawal_tengah']['simpul_awal']." AND simpul_tujuan=".$array['koordakhir_tengah']['simpul_awal']);
			
			//echo '<pre>'; print_r($array['titikbaru']);
		...
		function main($lat,$lng,$simpulbaru)
		{
			...
			// echo '<pre>'; print_r($titikterdekatverteks);
			// echo '<pre>'; print_r($potongjalur);
			//$this->pemotongansimpul($titikterdekatverteks);
			return $simpandtbase;
		}
	}

	// $obj=new simpulbaru;
	// $x=$obj->main(-0.923705,119.864846,1001);
	//echo '<pre>'; print_r($x);
